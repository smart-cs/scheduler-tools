...
class TestCourseModule(unittest.TestCase):
    def test_append_perm(self):
        result = list(schedulecreator.append_perm([], ['a', 'b', 'c']))
        expected = [['a'], ['b'], ['c']]
        self.assertCountEqual(expected, result)
        self.assertListEqual(expected, result)

        result = list(schedulecreator.append_perm([['a'], ['b'], ['c']], ['1', '2', '3', '4']))
...
class TestScheduleCreator(unittest.TestCase):
    ...
    def test_create_from_course_names(self):
        result = schedulecreator.create_from_course_names(['MATH 100', 'GRSJ 230'], self.db)
